...
const Header = () => {
  ...
  const menuItems = [
    ...
    { label: "Accomplishments", href: "/accomplishments", isExternal: false, icon: <PiCertificate size={18} /> },
 ...
 const handleToggle=()=>{
  ...
  return (
    <nav className="lg:flex items-center border-gray-200 mx-0 lg:px-24 fixed top-0 left-0 w-full h-24 z-40 bg-[#F4FAFF] shadow-lg dark:bg-[#0c121b]">
      <div className="w-full flex lg:flex-nowrap flex-wrap items-center justify-between  my-0 py-4 mx-0">
        ...
        <div className="flex  items-center lg:hidden justify-end  md:absolute right-2 w-1/2  gap-2">
  {/* mobile  toggle langButton */}
           <Button  className='' onClick={handleToggle} variant="dark" size="icon">
      ...
      <DropdownMenuContent align="end">
        ...
        <DropdownMenuItem onClick={() => handleLanguageChange("en")}>
          ...
          <button
            className=" inline-flex items-center p-2 w-11 justify-center text-sm rounded-lg lg:hidden hover:text-gray-100 focus:outline-none text-[#FBFDFF]"
            onClick={() => setShowMenuMobile(!showMenuMobile)}
            aria-expanded={showMenuMobile}
          >
            <span className="sr-only">Open main menu</span>
            {!showMenuMobile ? (
              <svg className="w-8 h-8 text-[#113264] dark:text-[#F4FAFF]" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 17 14">
        ...
        <div className="w-full md:flex items-center md:w-auto md:gap-5">
          <AnimatePresence>
            {showMenu && (
              <motion.ul
                className="bg-[#F4FAFF] hidden lg:flex  bg-transparent  font-medium flex-col p-4 md:p-0 border border-gray-100 rounded-lg md:flex-row md:space-x-8 rtl:space-x-reverse md:mt-0 md:border-0 dark:border-gray-700 dark:bg-[#0c121b] z-40 text-center"
...
{/* desktop toggle langButton */}
  ...
  onClick={handleHire} // Gérez l'état directement ici
>
  <span
    className="absolute right-0 -mt-12 h-32 w-8 translate-x-12 rotate-12 bg-[#F4FAFF] opacity-10 transition-all duration-1000 ease-out group-hover:-translate-x-40"
  ></span>
  ...
  <div className="flex items-center">
    ...
    <span className="ml-1 text-[#FBFDFF] flex items-center gap-2">
      <LiaHireAHelper size={20} /> Hire me
            ...
            {showMenuMobile && (
              <motion.ul
                className="relative bg-[#F4FAFF] lg:hidden lg:bg-transparent  font-medium flex flex-col p-4 md:p-0 border border-gray-100 rounded-lg rtl:space-x-reverse md:mt-0  dark:border-gray-700 dark:bg-[#0c121b] z-40 text-center"
                initial={{ opacity: 0, y: -10 }}
                animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0, y: -10 }}
                transition={{ duration: 0.3 }}
              >
                        <LuPanelTopClose onClick={() => setShowMenuMobile(!showMenuMobile)} className=" cursor-pointer text-[#113264] dark:text-[#F4FAFF] absolute top-2 right-5" size={28} />
                      {menuItems.map((item, index) => (
     <motion.li
  key={index}
  initial={{ opacity: 0 }}
  ...
  <div className="flex items-center">
    ...
    <span className="ml-1 text-[#FBFDFF] flex items-center gap-2">
      ...
      <AnimatePresence>
            {showMenuMobile && (
              <motion.ul
                className=" md:w-6/12 md:float-end relative bg-[#F4FAFF] lg:hidden lg:bg-transparent  font-medium flex flex-col p-4 border border-gray-100 rounded-lg rtl:space-x-reverse md:mt-0  dark:border-gray-700 dark:bg-[#0c121b] z-40 md:gap-3 md:p-5 "
                initial={{ opacity: 0, y: -10 }}
                animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0, y: -10 }}
                transition={{ duration: 0.3 }}
