...
const Presentation = () => {
 ...
 const show = useShowContext();
  ...
  return (
    <>
      <motion.div
        initial={{ opacity: 0 }} // Initialisation de l'opacité à 0
        whileInView={{ opacity: 1 }} // Animation de l'opacité à 1 lorsque l'élément devient visible
      ...
      >
        ...
        <motion.div
          ...
          animate={{ opacity: 1, x: 0 }}
          transition={{ duration: 0.8 }}
          className=" xl:w-8/12 2xl:w-6/12  flex flex-col justify-center lg:items-start items-center lg:ps-10 lg:pe-0 gap-2 order-3 lg:order-1"
        >
          <h2 className="text-xl gradient-text font-bold ">
            {show.language == "en" ? `Hey there! I'm` : `Salut ! Moi, c’est...`}
          </h2>
          <h1 className="gradient-text text-2xl lg:text-6xl my-2 text-center font-bold text-nowrap ">
            Aymane Rachid
          </h1>
          <p className=" w-full leading-6 text-md md:px-0 md:text-start text-center px-5 my-3 lg:my-2 font-bold dark:font-normal text-[#113264] dark:text-[#FBFDFF] ">
            {show.language == "en" ? (
              <>
                <span>
                  {" "}
                  Full-Stack Developer specializing in creating modern, scalable
                  web applications. I excel in technologies like React, Laravel,
                  and Node.js, with a focus on clean code and seamless user
                  experiences
                </span>{" "}
                <br />
                <span>
                  {" "}
                  As the founder of FSCodersHub, I lead and collaborate on
                  innovative tech projects, always striving to stay ahead of
                  emerging trends. Check out my work and let's bring your ideas
                  to life!
                </span>
              </>
            ) : (
              `Développeur Full-Stack spécialisé dans la création d'applications web modernes avec React, Laravel et Node.js. Fondateur de FSCodersHub, je dirige des projets innovants et m'efforce de produire un code propre et des expériences utilisateur fluides, tout en restant à l'avant-garde des tendances technologiques.`
            )}
          </p>
          ...
          >
            ...
            >
              <span className="absolute right-0 -mt-12 h-32 w-8 translate-x-12 rotate-12 bg-[#F4FAFF] opacity-10 transition-all duration-1000 ease-out group-hover:-translate-x-40"></span>
          ...
          >
            <a
              href={
                show.language === "en"
                  ? "/Portfolio/file/cv/Aymane_Rachid_cv_en.pdf"
                  : "/Portfolio/file/cv/Aymane_Rachid_cv.pdf"
              }
              download={
                show.language === "en"
                  ? "Aymane_Rachid_cv_en.pdf"
                  : "Aymane_Rachid_cv.pdf"
              }
